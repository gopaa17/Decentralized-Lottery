...
export const CONTRACT_ADDRESS =
  "CCKU7P2AWHFUHWVAKTK7IA2Q54VBBGLE6OV7WC7VAA3HTYOREPCIXYJQ";
...
/**
 * Initialize the lottery contract (admin only).
 * Calls: init(admin: Address)
 */
export async function initLottery(caller: string, admin: string) {
  return callContract(
    "init",
...
/**
 * Enter the lottery.
 * Calls: enter(player: Address)
 */
export async function enterLottery(caller: string, player: string) {
  return callContract(
    "enter",
...
/**
 * Get all players in the lottery (read-only).
 * Calls: get_players() -> Vec<Address>
 * Returns: string[] of player addresses
 */
export async function getPlayers(caller?: string) {
  const result = await readContract(
    "get_players",
    [],
    caller
  );
  // Convert Vec<Address> to array of strings
 ...
 * Calls: pick_winner(caller: Address) -> Address
 * Returns: string address of the winner
 */
export async function pickWinner(caller: string, admin: string) {
